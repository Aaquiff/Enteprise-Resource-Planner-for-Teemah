...
select pr.rawMaterialId,p.name,r.name from Products_RawMaterials as pr,Products as p,raw_item_tab as r where pr.productId = p.productId and r.item_id = pr.rawMaterialId and pr.productId = 1;

select item_id,name,description from raw_item_tab;

select * from Products_RawMaterials;

SELECT w.workOrderId,w.productId,w.quantity,w.status,w.state,w.notes,w.exportPoint,w.startDate,w.endDate,w.reference,p.name FROM WorkOrders w,Products p WHERE w.workOrderId = 1 and w.productId = p.productId;
