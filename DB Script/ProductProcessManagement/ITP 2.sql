select pr.rawMaterialId as 'Item Id',r.name as 'Name',pr.quantity as 'Quantity' from Products_RawMaterials as pr,Products as p,raw_item_tab as r where pr.productId = p.productId and r.item_id = pr.rawMaterialId and pr.productId = 1;

SELECT productId as 'Product Id', name as 'Name',description as 'Description',notes as 'Notes' FROM Products WHERE productId = 1;

SELECT * from ProductReq;

SELECT * from  WorkOrders;

SELECT remarkId as 'Remark Id',title as 'Title',status as 'Status',priority as 'Priority',created as 'Created',notes as 'Notes',archived as 'Is Archived' from Remarks;
...
/*No of Work Orders in Months;*/
CREATE VIEW `Work_Orders_vs_Time` AS
SELECT Year(startDate) as 'Year', Month(startDate) as 'Month', Count(*) as 'Total'
FROM WorkOrders
GROUP BY Year(startDate), Month(startDate);

/*No of Products Manufactured in Months;*/

CREATE VIEW `Products_vs_Time` AS
SELECT Year(w.startDate) as 'Year', Month(w.startDate) as 'Month', p.name as 'Product',p.productId as 'ProductID', Count(*) as 'Total'
FROM WorkOrders w, Products p
WHERE w.productId = p.productId
GROUP BY Year(w.startDate), Month(w.startDate),p.productId;

/*Work Orders and Time Taken;*/
CREATE VIEW `Work_Orders_vs_Time_Taken` AS
select w.workOrderId as 'Work_Order_Id', p.name as 'Product', (w.endDate - w.startDate) as 'Duration'
FROM WorkOrders w, Products p
WHERE w.productId = p.productId AND status = 'Completed';

/* AVg Time taken to complete a product work oderr*/
CREATE VIEW `Products_vs_Time_Taken` AS
select p.productId as 'ProductID', p.name as 'Product_Name', AVG((w.endDate - w.startDate)) as 'Duration'
FROM WorkOrders w, Products p
WHERE w.productId = p.productId AND status = 'Completed'
GROUP BY p.productId;

/*List Of Remarks*/
CREATE VIEW `Remark_Report` AS
Select title as 'Title',status as 'Status',notes as 'Notes',priority as 'Priority',created as 'Creation_Date'
From Remarks;


Select * from Work_Orders_vs_Time;
Select * from Products_vs_Time;
Select * from Work_Orders_vs_Time_Taken;
Select * from Products_vs_Time_Taken;
Select * from Remark_Report;
...
Create Table Products_RawMaterials(
    ...
    rawMaterialId integer references raw_item_tab(item_id) ON DELETE CASCADE,
    quantity int,
    notes varchar(256),
...
Create Table WorkOrders(
	...
	reference int references ProductReq ON DELETE CASCADE
);
...
Create Table TransportReq(
	...
	status varchar(32),
	notes varchar(256),
	reference int references WorkOrders ON DELETE CASCADE
);
...
Create Table Remarks(
	...
	lastAction date,
	notes varchar(256),
	reference int references WorkOrder ON DELETE CASCADE,
    archived boolean
...
-- drop Table Product;
drop Table Products_RawMaterials;
drop Table RawMatReq;
drop Table Remarks;
drop Table ProductReq;
drop Table TransportReq;
...
select * from WorkOrders;
select * from ProductReq;
select * from TransportReq;
select * from RawMatReq;
select * from Remarks;
...
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,500,'2016-08-22','2016-09-22','Pending',null);
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,300,'2016-07-22','2016-10-10','Pending',null);
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,500,'2016-08-20','2016-09-22','Pending',null);
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,600,'2016-08-15','2016-11-22','Pending',null);
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,300,'2016-05-20','2016-12-22','Pending',null);
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,200,'2016-08-14','2016-10-25','Pending',null);
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,500,'2016-07-18','2016-09-22','Pending',null);
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,400,'2016-08-16','2016-09-10','Pending',null);
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,300,'2016-07-18','2016-10-15','Pending',null);
insert into ProductReq(productId,quantity,requestDate,orderDate,status,notes) values(1,400,'2016-08-20','2016-09-24','Pending',null);
